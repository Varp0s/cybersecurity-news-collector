...
WRIED_SECURITY_RSS = os.getenv("WRIED_SECURITY_RSS")

def get_wired_security_articles():
    wired_security_content = feedparser.parse(WRIED_SECURITY_RSS)
    all_articles = []

    for entry in wired_security_content.entries:
        title = entry.title
        link = entry.link
