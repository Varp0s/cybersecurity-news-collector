...
ZDNET_RSS = os.getenv("ZDNET_RSS")

def get_zdnet_articles():
    zdnet_content = feedparser.parse(ZDNET_RSS)
    all_articles = []

    for entry in zdnet_content.entries:
        title = entry.title
        link = entry.link
        ...
        article_paragraphs = soup.find_all('div', class_='c-ShortcodeContent')
